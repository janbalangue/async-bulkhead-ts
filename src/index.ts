...
type Waiter = {
  // resolve is only called if granted a token
...
export function createBulkhead(opts: BulkheadOptions) {
  ...
  const acquire = (ao: AcquireOptions = {}): Promise<AcquireResult> => {
    ...
    return new Promise<AcquireResult>((resolve) => {
      const w: Waiter = { resolve, cancelled: false };
  ...
  const run = async <T>(
    ...
    if (!r.ok) {
      ...
      // 1) throw a typed error (recommended), or
      // 2) return a Result<T> shape.
      const e = new Error(`Bulkhead rejected: ${r.reason}`);
      (e as any).code = "BULKHEAD_REJECTED";
      (e as any).reason = r.reason;
      throw e;
    }
    try {
